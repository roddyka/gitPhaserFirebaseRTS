...
function startAll(options, noUi = false) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (options.import) {
            const importFilePath = path.join(path.resolve(options.import), hubExport_1.HubExport.METADATA_FILE_NAME);
            if (fsutils_1.fileExistsSync(importFilePath)) {
                exportMetadata = JSON.parse(fs
                    .readFileSync(path.join(path.resolve(options.import), hubExport_1.HubExport.METADATA_FILE_NAME), "utf8")
                    .toString());
            }
            else {
                emulatorLogger_1.EmulatorLogger.forEmulator(types_1.Emulators.HUB).logLabeled("WARN", "emulators", `Import/Export metadata file does not exist. ${clc.bold("Skipping data import!")} Metadata file location: ${importFilePath}`);
            }
        }
        if (shouldStart(options, types_1.Emulators.FUNCTIONS)) {
            ...
            if (process.env.GOOGLE_APPLICATION_CREDENTIALS) {
                functionsLogger.logLabeled("WARN", "functions", `Your GOOGLE_APPLICATION_CREDENTIALS environment variable points to ${process.env.GOOGLE_APPLICATION_CREDENTIALS}. Non-emulated services will access production using these credentials. Be careful!`);
            }
            else {
                ...
                if (defaultCredPath) {
                    ...
                    functionsLogger.logLabeled("WARN", "functions", "You are not signed in to the Firebase CLI. If you have authorized this machine using gcloud application-default credentials those may be discovered and used to access production services.");
                }
            }
            ...
            const functionsEmulator = new functionsEmulator_1.FunctionsEmulator({
                ...
                env: Object.assign(Object.assign({}, credentialEnv), options.extensionEnv),
